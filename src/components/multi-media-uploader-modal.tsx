...
import { Upload, X, Camera, Film, Image as ImageIcon, Sparkles, CheckCircle2, Lock, Globe, Plus, Trash2, ArrowRight } from 'lucide-react';
  ...
  const handleUploadSubmit = async () => {
    ...
    try {
      ...
      const compressedFiles = await Promise.all(
        queuedFiles.map(async (item, idx) => {
          if (item.isVideo) return item.file;
          setUploadStatus(`Optimizing photo ${idx + 1} of ${queuedFiles.length}...`);
          return compressImageFile(item.file);
        })
      );

      setUploadPercent(50);
      setUploadStatus(`Uploading to live wedding gallery...`);

      const formData = new FormData();
      for (const file of compressedFiles) {
        formData.append('files', file);
      }
      formData.append('guestName', finalName);
      formData.append('visibility', visibility);
      if (selectedQuest) formData.append('questTag', selectedQuest);
      ...
      toast({
        ...
        description: visibility === 'public' ? 'Live on the Memories Wall.' : 'Saved secretly to Couple’s Vault.',
      });
      ...
      if (onUploadSuccess && Array.isArray(data.items)) {
        onUploadSuccess(data.items);
      }
