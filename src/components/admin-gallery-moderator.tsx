...
import { Trash2, Download, RefreshCw, Sparkles, Image as ImageIcon, Video, CheckCircle2, ShieldAlert, Search, X, CheckSquare, Square } from 'lucide-react';
...
export function AdminGalleryModerator() {
  ...
  const loadMedia = async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/media?all=true&limit=250');
  ...
  const photoCount = items.filter(i => !(i.imageUrl?.startsWith('data:video') || /\.(mp4|webm|mov)$/i.test(i.imageUrl || '') || i.mediaType === 'video')).length;
